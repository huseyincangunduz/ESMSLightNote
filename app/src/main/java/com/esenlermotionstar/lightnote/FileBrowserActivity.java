...
public class FileBrowserActivity extends Activity {
	...
	private void updateCurrentDirectoryTextView() {
		...
		((TextView) this.findViewById(R.id.currentDirectoryTextView))
				.setText(getString(R.string.curent_dir) + ": " + curDirString);
	...
	private void loadFileList() {
		try {
			...
			Log.e(LOGTAG, getString(R.string.unable_write_on_sd_card));
		}
		fileList.clear();
		...
		if (path.exists() && path.canRead()) {
			...
			for (int i = 0; i < fList.length; i++) {
				...
				Log.d(LOGTAG,
						getString(R.string.file_colon) + fList[i] + " " + getString(R.string.readable)+":"
				...
				/*else if (sel.isFile() && ESMSUtils.AcceptFile(sel,".png",".jpg",".jpeg"))
				{
					try
					{
						InputStream strm = new FileInputStream(sel);
			...
			if (fileList.size() == 0) {
				...
				fileList.add(0, new Item(getString(R.string.dir_is_empty), -1, true));
			} else {// sort non empty list
				Collections.sort(fileList, new ItemFileNameComparator());
			}
		} else {
			Log.e(LOGTAG, getString(R.string.does_not_exst_cannot_read));
		}
